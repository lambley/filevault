...
describe('FileUpload', () => {
  ...
  it('should show error if file name or file is not provided', async () => {
    ...
    await waitFor(() => {
      const noteInput = screen.getByPlaceholderText('Enter file name...') as HTMLInputElement;
  ...
  it('should upload file successfully', async () => {
    ...
    mock.onGet('/files').reply(200, [...mockFiles, { name: 'File 3', key: 'key3' }]);
  ...
  it('should handle file upload error', async () => {
    ...
    // testing html validation rather than custom error message
    await waitFor(() => {
      const noteInput = screen.getByLabelText(/choose file/i) as HTMLInputElement;
      expect(noteInput.validity.valueMissing).toBe(true);
    });
  });

...
describe('fileUtils', () => {
  ...
  describe('loadFilesData', () => {
    it('should load files data from existing file', () => {
      ...
      expect(fs.readFileSync).toHaveBeenCalledWith(path.resolve(__dirname, '../../filesData.json'));
    });
    ...
    it('should return an empty array if file does not exist', () => {
      ...
      expect(fs.existsSync).toHaveBeenCalledWith(path.resolve(__dirname, '../../filesData.json'));
  ...
  describe('saveFilesData', () => {
    it('should save files data to file', () => {
      ...
      const newData: FileMetadata[] = [{ name: 'NewDocument', key: 'newDocument.pdf' }];
    ...
    it('should overwrite existing data in file', () => {
      ...
      const newData: FileMetadata[] = [{ name: 'UpdatedDocument', key: 'updatedDocument.pdf' }];
      saveFilesData(newData);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        path.resolve(__dirname, '../../filesData.json'),
        JSON.stringify(newData, null, 2),
